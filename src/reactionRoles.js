...
export async function addMapping(guildId, messageId, emoji, roleId) {
  ...
  `, [guildId, messageId, key, roleId]);
  cache.set(`${guildId}:${messageId}:${key}`, roleId);
}

export async function removeMapping(guildId, messageId, emoji) {
  const key = normalizeEmojiKey(emoji);
  await pool.query('DELETE FROM reaction_roles WHERE guild_id = $1 AND message_id = $2 AND emoji_key = $3', [guildId, messageId, key]);
  cache.delete(`${guildId}:${messageId}:${key}`);
}

export async function getMapping(guildId, messageId, emoji) {
  await ensureLoaded();
  return cache.get(`${guildId}:${messageId}:${normalizeEmojiKey(emoji)}`) ?? null;
}

export async function listMappings(guildId, messageId) {
  await ensureLoaded();
  const prefix = `${guildId}:${messageId}:`;
  return [...cache.entries()].filter(([key]) => key.startsWith(prefix)).map(([key, role_id]) => ({ emoji_key: key.slice(prefix.length), role_id }));
}
