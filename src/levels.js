...
export async function awardMessageXp(guildId, userId, member, channelId) {
  ...
  state.textXp += randomInt(Number(settings.xp_min), Number(settings.xp_max));
  ...
  try {
    ...
    for (const item of dirty) {
      ...
      await client.query(`INSERT INTO guild_members (guild_id, user_id, text_xp, text_level, voice_xp, voice_level) VALUES ($1, $2, $3, $4, $5, $6)
        ON CONFLICT (guild_id, user_id) DO UPDATE SET text_xp = EXCLUDED.text_xp, text_level = EXCLUDED.text_level, voice_xp = EXCLUDED.voice_xp, voice_level = EXCLUDED.voice_level, updated_at = NOW()`,
...
export async function getRank(guildId, userId) {
  ...
  const { rows } = await pool.query(`SELECT
    ...
    FROM guild_members m WHERE m.guild_id = $1 AND m.user_id = $2`, [guildId, userId]);
...
export async function getLeaderboard(guildId, type = 'text', limit = 10) {
  ...
  if (type === 'combined') {
    const { rows } = await pool.query('SELECT user_id, text_xp + voice_xp AS xp, GREATEST(text_level, voice_level) AS level FROM guild_members WHERE guild_id = $1 ORDER BY (text_xp + voice_xp) DESC LIMIT $2', [guildId, safeLimit]);
    return rows;
  }
  const column = type === 'voice' ? 'voice_xp' : 'text_xp';
  const level = type === 'voice' ? 'voice_level' : 'text_level';
  const { rows } = await pool.query(`SELECT user_id, ${column} AS xp, ${level} AS level FROM guild_members WHERE guild_id = $1 ORDER BY ${column} DESC LIMIT $2`, [guildId, safeLimit]);
  return rows;
}
...
export async function setXp(guildId, userId, type, xp) {
  ...
  await pool.query(`INSERT INTO guild_members (guild_id, user_id, ${field}, ${levelField}) VALUES ($1, $2, $3, $4)
    ON CONFLICT (guild_id, user_id) DO UPDATE SET ${field} = EXCLUDED.${field}, ${levelField} = EXCLUDED.${levelField}, updated_at = NOW()`, [guildId, userId, value, levelFromXp(value)]);
...
export async function addReward(guildId, roleId, textLevel = 0, voiceLevel = 0) {
  await pool.query('INSERT INTO level_rewards (guild_id, role_id, text_level, voice_level) VALUES ($1, $2, $3, $4) ON CONFLICT (guild_id, role_id, text_level, voice_level) DO NOTHING', [guildId, roleId, textLevel, voiceLevel]);
...
function rewardSatisfied(reward, state) {
  return (Number(reward.text_level) === 0 || state.textLevel >= Number(reward.text_level)) && (Number(reward.voice_level) === 0 || state.voiceLevel >= Number(reward.voice_level));
}
...
export async function syncLevelRewards(guild, member, state = null) {
  ...
  if (settings.remove_lower_rewards && satisfied.length) {
    ...
    for (const reward of rewards) if (Math.max(Number(reward.text_level), Number(reward.voice_level)) < highest) targetIds.delete(reward.role_id);
  }
...
export async function announceLevelUp(guild, member, info) {
  ...
  const message = String(settings.levelup_message || 'Congratulations [user]! You reached **Level [level]**.')
    .replaceAll('[user]', `<@${member.id}>`).replaceAll('[user.id]', member.id).replaceAll('[user.username]', member.user.username)
    .replaceAll('[level]', String(info.level)).replaceAll('[oldLevel]', String(info.oldLevel)).replaceAll('[type]', info.type);
...
export async function shutdownLevels() {
  for (let attempt = 1; attempt <= 3; attempt++) {
    if (await flushXp()) { cooldowns.clear(); cache.clear(); settingsCache.clear(); return true; }
